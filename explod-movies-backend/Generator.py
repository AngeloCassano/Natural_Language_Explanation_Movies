...
# tipo schema (descrizione del film sotto forma di elenco di caratteristiche e proprietà)
def get_descrizione_film(item_raccomandato, recommendation):
    explanation = ""
    property_movies = get_property_movies(recommendation)    # richiamo la funzione che seleziona le proprietà delle raccomandazioni
    property_item = []
    subject_prop = []
    subject_prop_provv = {}
    for line in property_movies:
        if item_raccomandato == line[0]:              # confrontando l'item da raccomandare con tutte le raccomandazioni
            property_item.append(line)                # prendo solo le proprietà che si riferiscono all'item da raccomandare
    ...
    if len(property_item) > 0:                        # inizia qui la costruzione della spiegazione
        ...
        for prop_corrente in property_item:                                       # per ogni proprietà dell'item raccomandato
            subject = prop_corrente[1]
            subject = pulisci_subject(subject)                                    # pulisco l'URI del tipo di proprietà
            if subject != "rdf-schema#seeAlso" and subject != "openingFilm" and subject != "soundRecording" and subject != "subject" and subject != "film" and subject != "closingFilm" and subject != "owl#differentFrom" and subject != "releaseLocation":
                prop_corrente_pulita = pulisci_uri_prop(prop_corrente[2])         # pulisco l'uri della proprietà
                list = [subject, prop_corrente_pulita]
                subject_prop.append(list)
                subject_prop_provv[subject] = prop_corrente_pulita

        for key, value in subject_prop_provv.items():       # per ogni propietà mi costruisco le righe della descrizione
            prop_subject = []
...
# Vengono effettuate le fasi centrali del Generator di accorpamento e filtraggio delle proprietà per evitare ripetizioni
# Questa funzione è specifica per quando c'è solo un item raccomandato
        ...
        if len(new_architecture_cleaned) > 1:
            for i in range(1, limit):                                     # fase di filtraggio
                ...
                for j in range(i):
                    ...
                    old_property = pulisci_property(old_property)         # viene pulito l'URI della seconda proprietà
                    if current_property == old_property:                  # se le due proprietà sono uguali
                        add = False                                       # la seconda non verrà presa in considerazione
                if add:
        ...
        for k in range(len(list_prop_cons)):                              # fase di accorpamento
            ...
            for j in range(k + 1, len(list_prop_cons)):
                ...
                if recI == recK and profI == profK and profI not in considered_items:
                    properties += "-and-" + properties1         # proprietà in comune tra gli item vengono messe insieme
...
# Funzione che ottimizza la struttura dati creata precedentemente per poter generare la spiegazione
# Vengono effettuate le fasi centrali del Generator di accorpamento e filtraggio delle proprietà per evitare ripetizioni
# Questa funzione è specifica per quando ci sono più item raccomandati e ha lo stesso funzionamento della precedente
...
# Si possono scegliere due tipi di template per generare la spiegazione e la possibilità di inserire tag html
# per la visualizzazione degli items e delle proprietà in una pagina web
...
# Funzione che pulisce l'URI del tipo di proprietà lasciando solo il tipo
...
# Funzione che pulisce l'URI di una proprietà lasciando solo il nome della proprietà
...
# Funzione che pulisce l'URI di una proprietà per essere inserita nella struttura dati prima di generare la spiegazione
...
# Funzione che recupera la lista delle proprietà dalla struttura dati creata e che verranno usate nella spiegazione
...
# Funzione che sceglie l'avverbio di gradimento in modo dinamico in base al numero di volte che la proprietà
...
# che la proprietà appare nel profilo dell'utente
...
# Funzione che carica dal file di mapping le proprietà con i rispettivi tipi
...
# Funzione che prende e inserisce nella frase il tipo della proprietà che si sta considerando e che recupera dai file
# di mapping, insieme al predicato corrispondente al tipo di proprietà
# Viene fatta anche la pulizia degli URI
def get_subject_prop(prop_corrente):
    properties_items = carica_properties_item()            # carico le proprietà con il rispettivo tipo
